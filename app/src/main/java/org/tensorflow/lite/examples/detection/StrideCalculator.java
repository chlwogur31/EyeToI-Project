...
public class StrideCalculator {
    ...
    public void cal_PDR() {
        // 1. 가속도를 벡터합으로 계산 x, y, z 벡터 합성
        cal_Vector();
        // 2. 고주파 필터링(High-pass Filtering (HPF)) => HPF를 통해 중력 값을 제거한다.
        cal_HPF();
        //  3. 저주파 필터링 =>   -저주파 필터링(Low-pass Filtering (LPF))
//             : LPF를 통해 노이즈 값을 제거한다.
//              HPF를 거친 값의 10개 평균을 계산한다.
        ...
        /*
         *  4. Zero-crossing Method와 WeinBerg Approach
    ...
    public void cal_StepSize()
    {
        ...
        // TARGET : stride = temp, will need to calc mean
        // Weinberg Approach
        temp = k* Math.pow(max-min, 1.0/4.0);

        // 정지 상태일때의 값 한번더 필터,  0.2 ~ 1.0 사이 값으로 인정
        if(temp >= 0.2 && temp <= 1.0)
            distance += temp;

        // For Test
//        setCurrentStride(temp);
    ...
    public double getStride(){
        System.out.println("distance = "+ distance + ", stepcnt : "+step);
        return distance*100/step;
    }
//    public void setCurrentStride(double temp){ currentStride = temp;}
//    public int getCurrentStride(){return (int)currentStride*100;}
}
